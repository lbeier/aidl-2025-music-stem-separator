...
def analyze_audio_separation(mix_path, vocals_path, instruments_path, reference_vocals_path=None, reference_instruments_path=None):
    ...
    instruments = load_audio(instruments_path)

    # Load reference files if provided
    ref_vocals = load_audio(
        reference_vocals_path) if reference_vocals_path else None
    ref_instruments = load_audio(
        reference_instruments_path) if reference_instruments_path else None
    ...
    print(f"Mix duration: {len(mix)/44100:.2f} seconds")
    ...
    # Spectral analysis
    print("\n2. Spectral Analysis:")
    mix_spec = compute_spectrogram(mix)
    vocals_spec = compute_spectrogram(vocals)
    instruments_spec = compute_spectrogram(instruments)
    ...
    print(f"Mix frequency range: {np.argmax(mix_freq)*44100/2048:.1f} Hz")
    print(
        f"Vocals frequency range: {np.argmax(vocals_freq)*44100/2048:.1f} Hz")
    print(
        f"Instruments frequency range: {np.argmax(instruments_freq)*44100/2048:.1f} Hz")
    ...
    if ref_vocals is not None and ref_instruments is not None:
        ...
        try:
            sdr_vocals, _, _, _ = mir_sep.bss_eval_sources(
                ref_vocals, vocals)
    ...
    # Waveform plot
    plt.subplot(3, 1, 1)
    plt.plot(mix[:44100])  # First second
    ...
    plt.subplot(3, 1, 3)
    mel_spec = compute_mel_spectrogram(mix)
    librosa.display.specshow(mel_spec, sr=44100, hop_length=512,
