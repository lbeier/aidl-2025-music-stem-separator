...
SPECTROGRAM_OUTPUT_PATH = "{}/sample_data/spectrograms_various"  # Different output folder
WAVEFORM_OUTPUT_PATH = "{}/sample_data/waveforms_various"  # Different output folder
        ...
        if "train" in str(file):
            ...
            for i in range(N_SYNTHETIC_SONGS):
                try:
                    ...
                    synthetic_versions.append((synthetic, f"synthetic_mix_{i+1}"))
                except Exception as e:
                    print(f"Warning: Failed to create synthetic song {i+1}: {e}")
        
        # Process original version and synthetic versions
        versions_to_process = [(mix, "original")] + synthetic_versions
        
        for audio_data, version_name in versions_to_process:
            suffix = f"_{version_name}" if version_name != "original" else ""
            
            # MEL spectrograms
            mel_norm, mel_db = compute_normalized_mel(audio_data, sr)
            np.save(spectrogram_dir / f"{file.stem}{suffix}_mix_mel.npy", mel_norm)
            save_spectrogram_image(mel_db, spectrogram_dir / f"{file.stem}{suffix}_mix_mel.png", sr)
            
            # STFT spectrograms
            stft_norm, stft_db = compute_normalized_stft(audio_data, sr)
            np.save(spectrogram_dir / f"{file.stem}{suffix}_mix_stft.npy", stft_norm)
            save_spectrogram_image(stft_db, spectrogram_dir / f"{file.stem}{suffix}_mix_stft.png", sr)
            
            # Waveform
            save_waveform_image(audio_data, waveform_dir / f"{file.stem}{suffix}_mix_waveform.png", sr)
        
        print(f"Finished {file.name} with mixed synthetic versions")
