    ...
    try:
        ...
        try:
            ...
            ax.text(
                0.5,
                0.5,
                "Spectrogram visualization failed",
...
def process_track_directory(track_dir: Path, spectrogram_dir: Path, waveform_dir: Path):
    ...
    Loads stems, combines instruments, calculates STFT spectrograms, and saves outputs.
    """
    ...
    try:
        for stem_name in REQUIRED_STEMS:
            ...
            )  # Load as potentially stereo, sr=None to preserve original SR

            if target_sr is None:
                target_sr = sr
            elif sr != target_sr:
                print(
                    f"  Warning: Sample rate mismatch in {track_name} ({stem_name}.wav has {sr} Hz, expected {target_sr} Hz). Skipping track."
                )
        ...
        print(
            f"  Debug: Loaded 'mixture.wav' - Duration: {mixture_duration_seconds:.2f}s, Samples: {mixture_duration_samples}, Shape: {stems['mixture'].shape}"
        )
        # --- End Debug ---
        ...
        instruments_signal = np.zeros(
            (
                max_len,
                stems[INSTRUMENT_STEMS[0]].shape[1] if stems[INSTRUMENT_STEMS[0]].ndim > 1 else 1,
            ),
            dtype=np.float32,
        )
        ...
        for name in INSTRUMENT_STEMS:
            ...
            if signal.shape[0] < max_len:
                ...
                pad_width = (
                    ((0, max_len - signal.shape[0]), (0, 0)) if signal.ndim > 1 else ((0, max_len - signal.shape[0]))
                )
        ...
        if mix_signal.shape[0] != max_len:
            pad_width = (
                ((0, max_len - mix_signal.shape[0]), (0, 0))
                if mix_signal.ndim > 1
                else ((0, max_len - mix_signal.shape[0]))
            )
            mix_signal = (
                np.pad(mix_signal, pad_width, mode="constant")
                if mix_signal.shape[0] < max_len
                else mix_signal[:max_len]
            )
        if vocals_signal.shape[0] != max_len:
            pad_width = (
                ((0, max_len - vocals_signal.shape[0]), (0, 0))
                if vocals_signal.ndim > 1
                else ((0, max_len - vocals_signal.shape[0]))
            )
            vocals_signal = (
                np.pad(vocals_signal, pad_width, mode="constant")
                if vocals_signal.shape[0] < max_len
                else vocals_signal[:max_len]
            )
        ...
        # --- Generate STFT Spectrograms (using MONO versions explicitly) ---
        sr = target_sr  # Use the confirmed sample rate

        # Calculate full STFT spectrograms
        ...
        # --- Sanity Check: Ensure STFT Spectrograms are 2D ---
        if mix_stft_norm_full.ndim != 2 or vocals_stft_norm_full.ndim != 2 or instruments_stft_norm_full.ndim != 2:
            print(f"  ################# ERROR ##################")
            print(f"  STFT spectrogram calculation did not result in 2D arrays for track {track_name}.")
            print(
                f"  Shapes - STFT(Mix/Voc/Inst): {mix_stft_norm_full.shape}, {vocals_stft_norm_full.shape}, {instruments_stft_norm_full.shape}"
            )
        ...
        time_axis = 1
        frames_dim = mix_stft_norm_full.shape[time_axis]

        print(f"  Full STFT spectrogram shape: {mix_stft_norm_full.shape}")  # Should show (freq, time)
        ...
        print(
            f"  Track length: {frames_dim} frames. Chunk length: {chunk_length_frames} frames. Num chunks: {num_chunks}"
        )

        if num_chunks == 0:
            print(
                f"  Warning: Track {track_name} is shorter than chunk duration ({CHUNK_DURATION_SECONDS}s). Skipping."
            )
            return

        # --- Save Chunked STFT Outputs ---
        for i in range(num_chunks):
            ...
            assert mix_stft_chunk.ndim == 2, f"Mix STFT chunk {i} is not 2D!"

            np.save(
                spectrogram_dir / f"{track_name}_mix_stft_{chunk_suffix}.npy",
                mix_stft_chunk,
            )
            np.save(
                spectrogram_dir / f"{track_name}_vocals_stft_{chunk_suffix}.npy",
                vocals_stft_chunk,
            )
            np.save(
                spectrogram_dir / f"{track_name}_instruments_stft_{chunk_suffix}.npy",
                instruments_stft_chunk,
            )
        ...
        total_chunk_duration_seconds = librosa.frames_to_time(
            total_chunk_frames, sr=sr, hop_length=HOP_LENGTH, n_fft=N_FFT
        )
        ...
        # --- Optional: Save full STFT spectrogram PNGs for overall visualization ---
        try:
            save_spectrogram_image(
                mix_stft_db_full,
                spectrogram_dir / f"{track_name}_mix_stft_FULL.png",
                sr,
            )
            save_spectrogram_image(
                vocals_stft_db_full,
                spectrogram_dir / f"{track_name}_vocals_stft_FULL.png",
                sr,
            )
            save_spectrogram_image(
                instruments_stft_db_full,
                spectrogram_dir / f"{track_name}_instruments_stft_FULL.png",
                sr,
            )
        except Exception as e:
            print(f"  Warning: FULL STFT Spectrogram visualization failed for {track_name}: {e}")
        ...
        try:
            ...
            save_waveform_image(
                vocals_signal,
                waveform_dir / f"{track_name}_vocals_waveform_FULL.png",
                sr,
            )
            save_waveform_image(
                instruments_signal,
                waveform_dir / f"{track_name}_instruments_waveform_FULL.png",
                sr,
            )
        except Exception as e:
        ...
        print(f"  Finished {track_name} ({num_chunks} STFT chunks saved)")
