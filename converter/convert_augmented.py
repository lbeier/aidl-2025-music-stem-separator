...
def process_file(file: Path, spectrogram_dir: Path, waveform_dir: Path):
    ...
    try:
        ...
        # Read only the stems we need (0: mixture, 3: accompaniment, 4: vocals)
        audio, sr = stempeg.read_stems(str(file), stem_id=[0, 3, 4])

        # Extract stems and preprocess (convert to mono and resample)
        ...
        versions_to_process = [(mix, "original")]
        
        # Create synthetic versions for training data
        if "train" in str(file):
            for i in range(N_SYNTHETIC_SONGS):
                try:
                    synthetic = create_synthetic_song(mix.copy(), sr)
                    versions_to_process.append((synthetic, f"synthetic_{i+1}"))
        ...
        print(f"Finished {file.name} with synthetic versions")
